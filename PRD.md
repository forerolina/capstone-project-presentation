...
A web-based appointment booking system that lets clients schedule appointments with a business, handles reminders, and collects payment upfront. It's for small service businesses that need to manage bookings, confirmations, and payments in one place. Clients book without authentication, while the business owner manages everything from a logged-in dashboard.
...
SvelteKit + Drizzle + Neon + Better Auth + Stripe
...
Scope: One appointment type, no rescheduling or cancellation, no reminders yet.
